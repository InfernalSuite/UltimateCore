...
public class HelpCommand extends HyperCommand {
    ...
    public void execute(CommandSender cs, String[] args) {
        ...
        int current = 0;
        p.sendMessage(com.infernalsuite.ultimatecore.skills.utils.StringUtils.color(HyperSkills.getInstance().getMessages().getMessage("helpHeader")));
        for (HyperCommand command : HyperSkills.getInstance().getCommandManager().commands) {
            if ((p.hasPermission(command.permission) || command.permission.equalsIgnoreCase("") || command.permission.equalsIgnoreCase("hyperskills.")) && command.isEnabled()) {
                if (current >= (page - 1) * 18 && current < page * 18)
                    p.sendMessage(com.infernalsuite.ultimatecore.skills.utils.StringUtils.color(HyperSkills.getInstance().getMessages().getMessage("helpMessage").replace("%command%", command.usage).replace("%description%", command.description)));
                current++;
            }
        }
        BaseComponent[] components = TextComponent.fromLegacyText(com.infernalsuite.ultimatecore.skills.utils.StringUtils.color(HyperSkills.getInstance().getMessages().getMessage("helpfooter").replace("%maxpage%", maxpage + "").replace("%page%", page + "")));
