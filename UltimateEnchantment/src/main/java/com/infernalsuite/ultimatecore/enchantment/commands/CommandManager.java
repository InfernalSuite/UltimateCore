...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public final List<mc.ultimatecore.enchantment.commands.Command> commands = new ArrayList<>();
    ...
    public CommandManager(String command) {
        EnchantmentsPlugin.getInstance().getCommand(command).setExecutor(this);
        EnchantmentsPlugin.getInstance().getCommand(command).setTabCompleter(this);
        registerCommands();
    }
     ...
     */
    public void registerCommand(mc.ultimatecore.enchantment.commands.Command command) {
    ...
    public boolean onCommand(CommandSender commandSender, Command cmd, String label, String[] args) {
        ...
        for (mc.ultimatecore.enchantment.commands.Command command : commands) {
            ...
            if (command.onlyForPlayers && !(commandSender instanceof Player)) {
                // Must be a player
                commandSender.sendMessage(Utils.color(EnchantmentsPlugin.getInstance().getMessages().getMessage("mustBeAPlayer")
                        .replace("%prefix%", EnchantmentsPlugin.getInstance().getConfiguration().prefix)));
                return false;
            }

            if (!((commandSender.hasPermission(command.permission) || command.permission
                    .equalsIgnoreCase("") || command.permission
                    .equalsIgnoreCase("hyperenchants.")) && command.enabled)) {
                // No permissions
                commandSender.sendMessage(Utils.color(EnchantmentsPlugin.getInstance().getMessages().getMessage("noPermission")
                        .replace("%prefix%", EnchantmentsPlugin.getInstance().getConfiguration().prefix)));
                return false;
            }
        ...
        commandSender.sendMessage(Utils.color(EnchantmentsPlugin.getInstance().getMessages().getMessage("unknownCommand")
                .replace("%prefix%", EnchantmentsPlugin.getInstance().getConfiguration().prefix)));
        return false;
    }
                ...
                for (String alias : command.aliases) {
                    if (alias.toLowerCase().startsWith(args[0].toLowerCase()) && (
                            command.enabled && (commandSender.hasPermission(command.permission)
                                    ...
                                    .equalsIgnoreCase("hyperenchants.")))) {
                        result.add(alias);
                    }
                }
            }
        ...
        for (mc.ultimatecore.enchantment.commands.Command command : commands) {
            if (command.aliases.contains(args[0]) && (command.enabled && (
                    commandSender.hasPermission(command.permission) || command.permission.equalsIgnoreCase("")
                            || command.permission.equalsIgnoreCase("hyperenchants.")))) {
