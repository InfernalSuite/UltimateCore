...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public void registerCommands() {
        ...
        registerCommand(new AddRegionCommand());
        registerCommand(new RegionListCommand());
        registerCommand(new RemoveRegionCommand());
            ...
            if (args.length != 0) {
                for (mc.ultimatecore.farm.commands.Command command : commands) {
                    if (command.getAliases().contains(args[0]) && command.isEnabled()) {
                        ...
                        if ((cs.hasPermission(command.getPermission()) || command.getPermission().equalsIgnoreCase("") || command.getPermission().equalsIgnoreCase("hyperregions.")) && command.isEnabled()) {
    ...
    public List<String> onTabComplete(CommandSender cs, Command cmd, String s, String[] args) {
        try {
            if (args.length == 1) {
                ...
                for (mc.ultimatecore.farm.commands.Command command : commands) {
                    for (String alias : command.getAliases()) {
                        if (alias.toLowerCase().startsWith(args[0].toLowerCase()) && (command.isEnabled() && (cs.hasPermission(command.getPermission()) || command.getPermission().equalsIgnoreCase("") || command.getPermission().equalsIgnoreCase("hyperregions.")))) {
                            result.add(alias);
                        }
                    }
                }
            ...
            for (mc.ultimatecore.farm.commands.Command command : commands) {
                if (command.getAliases().contains(args[0]) && (command.isEnabled() && (cs.hasPermission(command.getPermission()) || command.getPermission().equalsIgnoreCase("") || command.getPermission().equalsIgnoreCase("hyperregions.")))) {
