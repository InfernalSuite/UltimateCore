...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public final List<com.infernalsuite.ultimatecore.runes.commands.Command> commands = new ArrayList<>();
    ...
    public CommandManager(String command) {
        HyperRunes.getInstance().getCommand(command).setExecutor(this);
        HyperRunes.getInstance().getCommand(command).setTabCompleter(this);
        registerCommands();
    }
    ...
    public void registerCommands() {
        ...
        registerCommand(new GetRuneCommand());
        registerCommand(new RuneCommand());
        registerCommand(new RuneTableCommand());
     ...
     */
    public void registerCommand(com.infernalsuite.ultimatecore.runes.commands.Command command) {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        for (com.infernalsuite.ultimatecore.runes.commands.Command command : commands) {
            ...
            if (command.onlyForPlayers && !(commandSender instanceof Player)) {
                // Must be a player
                commandSender.sendMessage(StringUtils.color(HyperRunes.getInstance().getMessages().getMessage("mustBeAPlayer")
                        .replace("%prefix%", HyperRunes.getInstance().getConfiguration().prefix)));
                return false;
            }

            if (!((commandSender.hasPermission(command.permission) || command.permission
                    .equalsIgnoreCase("") || command.permission
                    .equalsIgnoreCase("hyperrunes.")) && command.enabled)) {
                // No permissions
                commandSender.sendMessage(StringUtils.color(HyperRunes.getInstance().getMessages().getMessage("noPermission")
                        .replace("%prefix%", HyperRunes.getInstance().getConfiguration().prefix)));
                return false;
            }
        ...
        commandSender.sendMessage(StringUtils.color(HyperRunes.getInstance().getMessages().getMessage("unknownCommand")
                .replace("%prefix%", HyperRunes.getInstance().getConfiguration().prefix)));
        return false;
    }
        ...
        for (com.infernalsuite.ultimatecore.runes.commands.Command command : commands) {
            if (command.aliases.contains(args[0]) && (command.enabled && (
                    commandSender.hasPermission(command.permission) || command.permission.equalsIgnoreCase("")
                            || command.permission.equalsIgnoreCase("hyperrunes.")))) {
