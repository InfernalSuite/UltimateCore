...
public class HelpCommand extends Command {
    ...
    public void execute(CommandSender cs, String[] args) {
        ...
        int current = 0;
        p.sendMessage(com.infernalsuite.ultimatecore.talismans.utils.StringUtils.color(HyperTalismans.getInstance().getMessages().getMessage("helpHeader")));
        for (Command command : HyperTalismans.getInstance().getCommandManager().commands) {
            if ((p.hasPermission(command.getPermission()) || command.getPermission().equalsIgnoreCase("") || command.getPermission().equalsIgnoreCase("hypertalisman.")) && command.isEnabled()) {
                if (current >= (page - 1) * 18 && current < page * 18)
                    p.sendMessage(com.infernalsuite.ultimatecore.talismans.utils.StringUtils.color(HyperTalismans.getInstance().getMessages().getMessage("helpMessage").replace("%command%", command.getUsage()).replace("%description%", command.getDescription())));
                current++;
            }
        }
        BaseComponent[] components = TextComponent.fromLegacyText(com.infernalsuite.ultimatecore.talismans.utils.StringUtils.color(HyperTalismans.getInstance().getMessages().getMessage("helpfooter").replace("%maxpage%", maxpage + "").replace("%page%", page + "")));
