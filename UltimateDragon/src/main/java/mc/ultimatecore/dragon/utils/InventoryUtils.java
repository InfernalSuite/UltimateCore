...
public class InventoryUtils {
    ...
    public static ItemStack makeItem(Item item, ItemStack stack) {
        try {
            ItemStack itemstack = stack != null ? makeItem(stack, item.amount, item.title, item.lore) : makeItem(item.material, item.amount, item.title, item.lore);
            if (item.material == XMaterial.PLAYER_HEAD && item.headData != null) {
                NBTItem nbtItem = new NBTItem(itemstack);
                NBTCompound skull = nbtItem.addCompound("SkullOwner");
                skull.setString("Name", "tr7zw");
