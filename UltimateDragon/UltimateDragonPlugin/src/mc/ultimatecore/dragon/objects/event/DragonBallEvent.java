...
import mc.ultimatecore.dragon.utils.particle.EnumParticle;
import mc.ultimatecore.dragon.utils.particle.Particle;
...
public class DragonBallEvent extends IDragonEvent {
    ...
    public DragonBallEvent(int duration, int repeat, double dragonSpeed, boolean freeze, double amount, boolean particle, double damage) {
        ...
        Bukkit.getScheduler().runTaskLater(HyperDragons.getInstance(), () -> {
            ...
            for(double radius = initialRadius; radius>0; radius-=0.1) {
                for (int i = 0; i < points; i++) {
                    ...
                    new Particle().sendParticle(Bukkit.getOnlinePlayers(), EnumParticle.DRAGON_BREATH, true, point.getWorld(), (float) point.getX(), (float) point.getY(), (float) point.getZ(), 0.0F, 0.0F, 0.0F, 0.0F, 15);
                }
            }
        }, 1);
    }
    ...
    private void shootFireball(Player player, int i) {
        ...
        Bukkit.getScheduler().runTaskLater(HyperDragons.getInstance(), () -> {
            if (location != null && this.enderDragon != null && player != null && player.isOnline()) {
                ...
                DragonFireball fb = this.armorStand.launchProjectile(DragonFireball.class, genVec(this.armorStand.getLocation(), loc));
                fb.setMetadata("DragonFireball", new FixedMetadataValue(HyperDragons.getInstance(), this.damage));
            }
        },20L * i);
    }
