...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public final List<com.infernalsuite.ultimatecore.trades.commands.Command> commands = new ArrayList<>();
    ...
    public CommandManager(String command) {
        HyperTrades.getInstance().getCommand(command).setExecutor(this);
        HyperTrades.getInstance().getCommand(command).setTabCompleter(this);
        registerCommands();
    }
    ...
    public void registerCommands() {
        registerCommand(new RemoveTradeCommand());
        registerCommand(new HelpCommand());
        registerCommand(new ReloadCommand());
        registerCommand(new MainMenuCommand());
        registerCommand(new SetupTradeCommand());
     ...
     */
    public void registerCommand(com.infernalsuite.ultimatecore.trades.commands.Command command) {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        for (com.infernalsuite.ultimatecore.trades.commands.Command command : commands) {
            ...
            if (command.onlyForPlayers && !(commandSender instanceof Player)) {
                // Must be a player
                commandSender.sendMessage(StringUtils.color(HyperTrades.getInstance().getMessages().getMessage("mustBeAPlayer")
                        .replace("%prefix%", HyperTrades.getInstance().getConfiguration().prefix)));
                return false;
            }

            if (!((commandSender.hasPermission(command.permission) || command.permission
                    .equalsIgnoreCase("") || command.permission
                    .equalsIgnoreCase("hypertrades.")) && command.enabled)) {
                // No permissions
                commandSender.sendMessage(StringUtils.color(HyperTrades.getInstance().getMessages().getMessage("noPermission")
                        .replace("%prefix%", HyperTrades.getInstance().getConfiguration().prefix)));
                return false;
            }
        ...
        commandSender.sendMessage(StringUtils.color(HyperTrades.getInstance().getMessages().getMessage("unknownCommand")
                .replace("%prefix%", HyperTrades.getInstance().getConfiguration().prefix)));
        return false;
    }
