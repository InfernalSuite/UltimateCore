...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public final List<mc.ultimatecore.anvil.commands.Command> commands = new ArrayList<>();
    ...
    public CommandManager(String command) {
        HyperAnvil.getInstance().getCommand(command).setExecutor(this);
        HyperAnvil.getInstance().getCommand(command).setTabCompleter(this);
        registerCommands();
    }

    /**
     * Registers all mc.ultimatecore.commands of this plugin.
     */
    public void registerCommands() {
        registerCommand(new HelpCommand());
        registerCommand(new ReloadCommand());
        registerCommand(new MainMenuCommand());
        registerCommand(new GiveBookItem());
     ...
     */
    public void registerCommand(mc.ultimatecore.anvil.commands.Command command) {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        for (mc.ultimatecore.anvil.commands.Command command : commands) {
            ...
            if (command.onlyForPlayers && !(commandSender instanceof Player)) {
                // Must be a player
                commandSender.sendMessage(StringUtils.color(HyperAnvil.getInstance().getMessages().getMessage("mustBeAPlayer")
                        .replace("%prefix%", HyperAnvil.getInstance().getConfiguration().prefix)));
                return false;
            }

            if (!((commandSender.hasPermission(command.permission) || command.permission
                    .equalsIgnoreCase("") || command.permission
                    .equalsIgnoreCase("hyperanvil.")) && command.enabled)) {
                // No permissions
                commandSender.sendMessage(StringUtils.color(HyperAnvil.getInstance().getMessages().getMessage("noPermission")
                        .replace("%prefix%", HyperAnvil.getInstance().getConfiguration().prefix)));
                return false;
            }
        ...
        commandSender.sendMessage(StringUtils.color(HyperAnvil.getInstance().getMessages().getMessage("unknownCommand")
                .replace("%prefix%", HyperAnvil.getInstance().getConfiguration().prefix)));
        return false;
    }
        ...
        for (mc.ultimatecore.anvil.commands.Command command : commands) {
            if (command.aliases.contains(args[0]) && (command.enabled && (
                    commandSender.hasPermission(command.permission) || command.permission.equalsIgnoreCase("")
                            || command.permission.equalsIgnoreCase("hyperanvil.")))) {
