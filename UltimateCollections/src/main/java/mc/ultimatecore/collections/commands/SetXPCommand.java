...
public class SetXPCommand extends Command {
    ...
    public SetXPCommand() {
        super(Collections.singletonList("setxp"), "Set xp to a player", "hypercollections.setxp", false, "/Collections setxp [Player] [Collection] [Amount]");
    }
            ...
            if (skill != null) {
                ...
                if (player != null) {
                    if (sender instanceof Player)
                        sender.sendMessage(StringUtils.color(HyperCollections.getInstance().getMessages().getMessage("setXP")
                                                                             .replace("%player%", player.getName())
                                                                             .replace("%collection%", skill.getKey())
                                                                             .replace("%quantity%", String.valueOf(quantity))
                                                                             .replace("%prefix%", HyperCollections.getInstance().getConfiguration().getPrefix())));
                    HyperCollections.getInstance().getApi().setXP(player, skill.getKey(), quantity);
                } else {
