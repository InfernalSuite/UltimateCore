...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public final List<mc.ultimatecore.collections.commands.Command> commands = new ArrayList<>();
    ...
    public CommandManager(String command) {
        HyperCollections.getInstance().getCommand(command).setExecutor(this);
        HyperCollections.getInstance().getCommand(command).setTabCompleter(this);
        registerCommands();
    }
    ...
    public void registerCommands() {
        registerCommand(new LevelsMenuCommand());
     ...
     */
    public void registerCommand(mc.ultimatecore.collections.commands.Command command) {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        for (mc.ultimatecore.collections.commands.Command command : commands) {
            ...
            if (command.onlyForPlayers && !(commandSender instanceof Player)) {
                // Must be a player
                commandSender.sendMessage(StringUtils.color(HyperCollections.getInstance().getMessages().getMessage("mustBeAPlayer")
            ...
            if (!((commandSender.hasPermission(command.permission) || command.permission
                    ...
                    .equalsIgnoreCase("hypercollections.")) && command.enabled)) {
                // No permissions
                commandSender.sendMessage(StringUtils.color(HyperCollections.getInstance().getMessages().getMessage("noPermission")
                                                                            .replace("%prefix%", HyperCollections.getInstance().getConfiguration().getPrefix())));
                return false;
            }
        ...
        commandSender.sendMessage(StringUtils.color(HyperCollections.getInstance().getMessages().getMessage("unknownCommand")
                                                                    .replace("%prefix%", HyperCollections.getInstance().getConfiguration().getPrefix())));
        return false;
    }
                ...
                for (String alias : command.aliases) {
                    if (alias.toLowerCase().startsWith(args[0].toLowerCase()) && (
                            command.enabled && (commandSender.hasPermission(command.permission)
                                    ...
                                    .equalsIgnoreCase("hypercollections.")))) {
                        result.add(alias);
                    }
                }
            }
        ...
        for (mc.ultimatecore.collections.commands.Command command : commands) {
            if (command.aliases.contains(args[0]) && (command.enabled && (
                    commandSender.hasPermission(command.permission) || command.permission.equalsIgnoreCase("")
                            || command.permission.equalsIgnoreCase("hypercollections.")))) {
